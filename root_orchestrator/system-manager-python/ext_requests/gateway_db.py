...
def mongo_update_node_services(node_id, service_id, port):
    return db.mongo_gateway_nodes.update(ObjectId(node_id), 
        { '$push' : { 'microservices': service_id, 'used_ports': port } })
...
def mongo_update_gateway_status(cluster_id, status, status_note):
    db.mongo_gateway_nodes.insert_one({'cluster_id': cluster_id,
                                        'status': status,
